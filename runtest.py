...
class TestSelector(unittest.TestCase):
    def setUp(self):
        pat = ["12 aa ba", "13 ab bb", "14 ac bc"]
...
class TestRegref(unittest.TestCase):
    def setUp(self):
        self.pmap = ["12 aa ba", "13 ab bb", "14 ac bc"]
        self.smap = ["12,aa,ba", "13,ab,bb", "14,ac,bc"]
        self.data = ["asaa id=12", "asdf id=14", "t aa id=42"]
    ...
    def test_search_and_replace_m_plus_n(self):
        reg = self._get_results(
            [self.pfile, "${1}", "<${3}>", "--where", "id=(\d+)", "1"], self.data
        )
        self.assertEqual(reg, ["asaa id=<ba>", "asdf id=<bc>", "t aa id=42"])

    def test_delete_m_plus_n(self):
        reg = self._get_results(
            [self.pfile, " id=${1}", "--where", "id=(\d+)", "1"], self.data
        )
